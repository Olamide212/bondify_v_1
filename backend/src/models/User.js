...
const userSchema = new mongoose.Schema(
  {
    ...
    drinking: {
      type: String,
      // Must match drinking-habits seed values exactly
      enum: ["No, I don't drink", 'Socially', 'Rarely', 'Regularly', 'Prefer not to say'],
    },
    smoking: {
      type: String,
      // Must match smoking-habits seed values exactly
      enum: ["No, I don't smoke", 'Socially', 'Occasionally', 'Regularly', 'Prefer not to say'],
    },
    exercise: {
      type: String,
      // Must match exercise-habits seed values exactly
      enum: ['Never', 'Rarely', 'Sometimes', 'Often', 'Daily'],
    },
    pets: {
      type: String,
      // Must match pets seed values exactly
      enum: ['I have pets', 'I want pets', "I don't want pets", 'Allergic to pets', 'Prefer not to say'],
    },
    children: {
      type: String,
    ...
    communicationStyle: {
      type: String,
      // Must match communication-style seed values exactly
      enum: ['Direct', 'Thoughtful', 'Emotional', 'Logical', 'Balanced'],
    },
    loveLanguage: {
      type: String,
      // Must match love-language seed values exactly
      enum: ['Words of Affirmation', 'Quality Time', 'Physical Touch', 'Acts of Service', 'Receiving Gifts'],
    },
    financialStyle: {
      type: String,
      // Must match financial-style seed values exactly
      enum: ['Spender', 'Saver', 'Investor', 'Balanced', 'Prefer not to say'],
    },
