 ...
 * app/(auth)/(onboarding)/verification/index.jsx
 *
 * Onboarding entry-point for selfie verification.
...
export default function VerificationScreen() {
  ...
  // Load profile photo + draft media for the intro step preview
  useEffect(() => {
    (async () => {
      try {
        const [draft, draftMainUrl] = await Promise.all([
          getOnboardingProfileMediaDraft(),
          getOnboardingDraftMainPhotoUrl(),
        ]);

        if (Array.isArray(draft)) setDraftMedia(draft);

        if (draftMainUrl) {
          setProfilePhotoUrl(draftMainUrl);
          return;
        }

        const profile = await profileService.getMyProfile({ force: true });
        const first   = Array.isArray(profile?.images) && profile.images.length > 0
          ? profile.images.sort((a, b) => (a.order ?? 0) - (b.order ?? 0))[0]
          : null;
        if (first?.url) setProfilePhotoUrl(first.url);
      } catch {
        // Silently fall back to shield icon
      }
    })();
  }, []);

  const handleCameraVisible = (isCamera) => {
    // Let the parent layout know so it can hide its own header if needed
    setVerificationStep(isCamera ? "camera" : "other");
  };
  ...
  return (
    <View style={s.root}>
      <VerificationFlow
        ...
        onBack={() => router.back()}
        onCameraVisible={handleCameraVisible}
      />
    </View>
  );
}
