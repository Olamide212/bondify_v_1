...
const MatchAvatar = ({ notification, onPress }) => {
  ...
  return (
    <Pressable onPress={() => onPress?.(notification)} className="items-center mr-4">
        ...
        <View className="w-16 h-16 rounded-full overflow-hidden bg-gray-700 border-2 border-primary">
          {avatarUrl ? (
            ...
            <View className="w-full h-full items-center justify-center bg-gray-300">
              <Text className="text-gray-400 text-2xl font-PlusJakartaSansBold">
                {name?.[0] || "?"}
              </Text>
            </View>
          )}
      ...
      <Text className="mt-2 text-xs text-gray-300 font-PlusJakartaSansMedium" numberOfLines={1}>
        {name || "Match"}
      </Text>
    </Pressable>
  );
...
const BoostButton = ({ onPress }) => (
  <Pressable onPress={onPress} className="items-center mr-4">
    ...
    <Text className="mt-2 text-xs text-gray-400 font-PlusJakartaSansMedium">Boost</Text>
  </Pressable>
);
      ...
      <View className="flex-row items-start">
        <View className="relative">
          <View className="w-12 h-12 rounded-full overflow-hidden bg-gray-700">
            {avatarUrl ? (
              ...
              <View className="w-full h-full items-center justify-center bg-gray-300">
                <Text className="text-gray-400 text-lg font-PlusJakartaSansBold">
                  {name?.[0] || "?"}
                </Text>
              </View>
            )}
        ...
        <View style={{flex: 1}} className="ml-3">
          <View className="flex-row items-center justify-between">
            <Text className="text-white font-PlusJakartaSansBold">
              <Text className="text-white">{name}</Text>
              <Text className="text-gray-400 font-PlusJakartaSans"> liked your photo</Text>
            </Text>
            <View className="flex-row items-center">
              <Text className="text-xs text-gray-400 font-PlusJakartaSans mr-1">
        ...
        <View style={{flex: 1}} className="ml-3">
          <View className="flex-row items-start justify-between">
            <View style={{flex: 1}}>
              <Text className="text-white font-PlusJakartaSansBold">
                {name}
                <Text className="text-gray-400 font-PlusJakartaSans"> invited you to</Text>
              </Text>
              {eventTitle && (
          ...
          <View className="flex-row mt-3 gap-2">
            ...
            >
              <Text className="text-gray-300 font-PlusJakartaSansMedium text-sm">Decline</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
      ...
      <View className="flex-row items-start">
        ...
        <View style={{flex: 1}} className="ml-3">
          <View className="flex-row items-center justify-between">
            <Text className="text-white font-PlusJakartaSansBold">
              AI Assistant:
              <Text className="text-gray-400 font-PlusJakartaSans"> {notification.body}</Text>
            </Text>
            <Text className="text-xs text-gray-400 font-PlusJakartaSans">
              {formatRelativeTime(notification.createdAt)}
            </Text>
          </View>
  ...
  return (
    ...
    >
      <View className="flex-row items-center">
        ...
        <View style={{flex: 1}} className="ml-3">
          <View className="flex-row items-center justify-between">
            <Text className="text-white font-PlusJakartaSansBold">{name || "Someone"}</Text>
            <View className="flex-row items-center">
              <Text className="text-xs text-gray-400 font-PlusJakartaSans mr-1">
                {formatRelativeTime(notification.createdAt)}
              </Text>
          ...
          <Text className="text-gray-400 font-PlusJakartaSans mt-1" numberOfLines={1}>
            "{notification.body}"
          </Text>
        </View>
      </View>
...
const SimpleNotificationCard = ({ notification, onPress }) => {
  ...
  return (
    ...
    >
      ...
      <View style={{flex: 1}} className="ml-3">
        <Text className="text-gray-300 font-PlusJakartaSans leading-5">
          {notification.body}
        </Text>
  ...
  return (
    ...
    >
      <View className="flex-row items-start">
        ...
        <View style={{flex: 1}} className="ml-3">
          <View className="flex-row items-center justify-between">
            <Text className="text-white font-PlusJakartaSansBold flex-1 mr-2" numberOfLines={1}>
          ...
          <Text className="text-gray-400 font-PlusJakartaSans mt-1" numberOfLines={2}>
            {notification.body}
          </Text>
        </View>
      </View>
        ...
        <SafeAreaView style={{flex: 1}} className="bg-[#121212]">
          ...
          <View className="px-5 py-3 flex-row items-center justify-between bg-[#121212]">
            <View className="flex-row items-center">
                 ...
                 {/* Title */}
            <View className=" py-4">
              <Text className="text-3xl font-PlusJakartaSansBold text-white">Notifications</Text>
            </View>
            </View>
            ...
            {newMatches.length > 0 && (
              <View className="mb-6">
                <View className="px-5 flex-row items-center justify-between mb-3">
                  <Text className="text-xs font-PlusJakartaSansBold text-gray-400 tracking-wider">
                    NEW MATCHES
                  </Text>
          ...
          {notifications.length > 0 && (
            <View className="px-5 py-4 border-t border-gray-700 bg-[#121212] flex-row items-center justify-between">
              ...
              <TouchableOpacity onPress={onClearAll}>
                <Text className="text-gray-400 font-PlusJakartaSansMedium">Clear all</Text>
              </TouchableOpacity>
            </View>
          )}
        </SafeAreaView>
