...
const BloodGroupModal = ({
  ...
  const { options, loading } = useLookupOptions("blood-group");
  ...
  useEffect(() => {
    if (visible && initialSelected) {
      ...
      console.log('[BloodGroupModal] Selected:', trimmed, 'Options:', options.map(o => o.value));
  ...
  return (
    <BaseModal onClose={onClose} visible={visible}>
      <ModalHeader centerText="Blood Group" onClose={onClose} />
