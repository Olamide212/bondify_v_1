  ...
  const handleRewind = async () => {
    try {
      ...
      if (response.success && response.data?.profile) {
        ...
        setHomeSwipedProfiles((prev) => prev.filter(id => id !== rewoundProfile.id));

        // Store the rewound profile so we can set the index to it
        setLastRewoundProfile(rewoundProfile);

        // Disable rewind after using it
        setRewindAvailable(false);

        return rewoundProfile;
      }
    } catch (error) {
